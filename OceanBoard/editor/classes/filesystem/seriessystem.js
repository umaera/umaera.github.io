...
function restoreAllSeries() {
	const seriesList = JSON.parse(localStorage.getItem('ob-series-list') || '[]');
	...
	if (seriesList.length > 0) {
		...
		console.log('[Series System] Restored', seriesList.length, 'series from localStorage');
	} else {
...
function showEmptyState() {
	...
	const specialList = document.querySelector('.ob-special-section .ob-tree-list');
		...
		contentSection.innerHTML = `
			<div class="ob-welcome ob-empty-state ob-animate-fade-in">
				...
				<p class="ob-welcome-hint ob-animate-fade-in" style="animation-delay: 0.6s;">
					Click the <span class="material-icons-round" style="font-size: 1em; vertical-align: middle;">add</span> button on the left to create your first series
				</p>
			</div>
		`;
...
function loadActiveSeries() {
	...
	if (seriesList.length === 0) {
		console.log('[Series System] No series available - staying in empty state');
		return;
	}
	...
	if (savedSeriesId) {
		...
		const tile = document.querySelector(`[data-series-id="${savedSeriesId}"]`);
		if (tile) {
...
function setActiveSeries(tile) {
	...
	// Update active state
	document.querySelectorAll('.ob-series-tile').forEach(t => t.classList.remove('active'));
...
function handleSeriesRename(tile, seriesId, metadata) {
	...
	if (tile.classList.contains('active')) {
		document.querySelector('.ob-project-title-static').textContent = newName;
	}
...
function handleSeriesCoverChange(tile, seriesId, metadata) {
	...
	if (!metadata) {
		metadata = {
			...
			initials: tile.querySelector('.ob-series-tile-content')?.textContent || 'S',
			color: '#E43967',
			coverImage: null,
			createdAt: Date.now()
		};
...
function handleSeriesDelete(tile, seriesId) {
	...
	if (confirm(`Delete series "${metadata.name}" and all its content?`)) {
		if (confirm('This cannot be undone! Are you sure?')) {
			...
			// Remove all files
			const workspaceData = JSON.parse(localStorage.getItem('ob-workspace-' + seriesId) || '{}');
